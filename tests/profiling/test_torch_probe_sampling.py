...
import probing.profiling.types as profiling_types


def _reset_hook_offset() -> None:
    profiling_types.MODULE_CALL_OFFSET = 0
    profiling_types.CURRENT_MODULE = None
    profiling_types.CURRENT_STAGE = None


def _bump_hook_offset(mod, stage: str) -> None:
    profiling_types.MODULE_CALL_OFFSET += 1
    profiling_types.CURRENT_MODULE = id(mod)
    profiling_types.CURRENT_STAGE = stage
...
def test_anchor_pre_always_pairs_post_when_curr_mod_differs():
    ...
    _bump_hook_offset(root, "pre forward")
