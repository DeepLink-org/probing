//! Execute playbook steps against a probing endpoint (sql / api / config).
...
use super::loader::{build_context, expand_template, load_playbook, Playbook, PlaybookStep};
...
fn ensure_read_only_sql(sql: &str) -> Result<()> {
    ...
    Err(anyhow!("Only read-only SQL is allowed in playbooks"))
}
...
async fn run_api_step(ctrl: &ProbeEndpoint, step: &PlaybookStep) -> StepOutcome {
...
async fn run_step(
    ...
    match step.step_type.as_str() {
        ...
        "config" => StepOutcome::Skipped {
            step_id: step.id.clone(),
            title: step.title.clone(),
            reason: "config steps are not applied automatically in CLI doctor".to_string(),
        },
...
pub fn list_playbooks() -> Result<()> {
    use super::loader::list_playbook_ids;
    println!("Available diagnostic playbooks:\n");
    for id in list_playbook_ids() {
        let pb = load_playbook(&id)?;
...
pub async fn run_doctor(
    ...
    if had_error {
        anyhow::bail!("playbook finished with errors");
    }
    Ok(())
}
