...
impl QueryResponseDto {
    ...
    pub fn error(code: String, message: String) -> Self {
        Self {
            payload: QueryDataDto::Error { code, message: message.clone() },
            timestamp: Self::now(),
            success: false,
...
impl From<crate::protocol::query::Query> for QueryRequestDto {
    fn from(query: crate::protocol::query::Query) -> Self {
        Self {
            expr: query.expr,
            opts: query.opts.map(|opts| QueryOptionsDto {
                limit: opts.limit,
            }),
        }
    }
...
impl From<QueryRequestDto> for crate::protocol::query::Query {
    fn from(dto: QueryRequestDto) -> Self {
        Self {
            expr: dto.expr,
            opts: dto.opts.map(|opts| crate::protocol::query::Options {
                limit: opts.limit,
            }),
        }
    }
    ...
    fn from(data: crate::protocol::query::Data) -> Self {
        match data {
            ...
            crate::protocol::query::Data::DataFrame(df) => {
                let cols = df.cols.into_iter().map(|seq| {
                    match seq {
            ...
            crate::protocol::query::Data::TimeSeries(ts) => {
                ...
                let cols = ts.cols.into_iter().map(|series| series.iter().map(convert_ele).collect()).collect();
