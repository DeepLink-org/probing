...
use probing_proto::protocol::query::{Query as ProtoQuery, Data as ProtoData};
...
#[axum::debug_handler]
pub async fn query_dto(axum::extract::Json(request_dto): axum::extract::Json<probing_proto::dto::query::QueryRequestDto>) -> impl IntoResponse {
    handle_query_dto(request_dto).await
}

/// Handle query DTO processing and convert to internal format
async fn handle_query_dto(request_dto: probing_proto::dto::query::QueryRequestDto) -> impl IntoResponse {
    ...
    match serde_json::to_string(&message) {
        Ok(json_request) => {
            process_engine_query(json_request).await
        },
        Err(e) => {
            (StatusCode::BAD_REQUEST, 
             format!("Failed to serialize request: {}", e)).into_response()
        }
    }
}
...
async fn process_engine_query(json_request: String) -> axum::response::Response {
    match crate::engine::query(json_request).await {
        Ok(response_json) => {
            convert_engine_response_to_dto(response_json).await
        },
        Err(api_error) => {
            convert_engine_error_to_dto(api_error).await
        }
    }
}
        ...
        Ok(message_response) => {
            ...
            match serde_json::to_string(&response_dto) {
                Ok(dto_response_json) => {
                    (StatusCode::OK, dto_response_json).into_response()
                },
                Err(e) => {
                    (StatusCode::INTERNAL_SERVER_ERROR, 
                     format!("Failed to serialize DTO response: {}", e)).into_response()
                }
            }
        },
        Err(e) => {
            (StatusCode::INTERNAL_SERVER_ERROR, 
             format!("Failed to parse engine response: {}", e)).into_response()
        }
    }
}

/// Convert engine error to DTO error response
async fn convert_engine_error_to_dto(api_error: crate::server::error::ApiError) -> axum::response::Response {
    ...
    match serde_json::to_string(&error_response) {
        ...
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, 
                  format!("Failed to serialize error response: {}", e)).into_response(),
    }
}
