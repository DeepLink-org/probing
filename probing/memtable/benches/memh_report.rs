...
impl BenchResult {
    fn total_ops(&self) -> u64 { self.ops_per_iter * self.iterations }
...
where F: FnMut() -> u64 {
    ...
    while warmup_start.elapsed() < WARMUP { black_box(f()); }
    ...
    BenchResult { name, ops_per_iter, iterations, elapsed: start.elapsed() }
}
...
fn build_fragmented_str_buf(keys: &[String], vals: &[String]) -> Vec<u8> {
    ...
    for (k, v) in keys.iter().zip(vals.iter()) { w.insert(k, &Value::Str(v)).unwrap(); }
...
fn get_r<'a>(results: &'a [BenchResult], name: &str) -> &'a BenchResult {
    results.iter().find(|r| r.name == name).unwrap_or_else(|| panic!("missing: {name}"))
}

fn print_section(results: &[BenchResult], title: &str, rows: &[(&str, &str)]) {
    println!("{title}");
    println!("  {:<42} {:>8} {:>12} {:>12}", "case", "iters", "ns/op", "M ops/s");
    println!("  {}", "-".repeat(80));
    for &(name, note) in rows {
        let r = get_r(results, name);
        println!(
            "  {:<42} {:>8} {:>12.2} {:>12.2}  {}",
            r.name, r.iterations, r.ns_per_op(), r.mops_per_sec(), note
        );
    }
    println!();
}
...
fn print_report(results: &[BenchResult]) {
    println!();
    println!("═══════════════════════════════════════════════════════════════");
    println!(" MEMH v3  N={N}  buckets={NUM_BUCKETS}  arena={}KB",
        ARENA_CAP / 1024);
    println!("═══════════════════════════════════════════════════════════════");
    println!();

    print_section(results, "insert", &[
        ("insert/new-scalar",          "fresh table; PUT_INLINE record + slot commit"),
        ("insert/update-scalar-inline","same key; update slot.val_bytes, zero arena writes"),
        ("insert/new-str",             "fresh table; PUT record + slot commit"),
        ("insert/update-str",          "same key; new PUT record each call  [incl. ~2MB reset/iter]"),
    ]);

    print_section(results, "get", &[
    ...
    print_section(results, "iter", &[
        ("iter/clean",      &format!("N={N} live records, no stale records")),
        ("iter/fragmented", &format!("N={N} live + {FRAG_UPDATES}×N stale, liveness check overhead")),
    ]);

    print_section(results, "baseline (hash only)", &[
    ...
    println!("scalar-inline speedup vs str-update: {:.1}×", stu.ns_per_op() / su.ns_per_op());
    let ic = get_r(results, "iter/clean");
    let ifr = get_r(results, "iter/fragmented");
    println!("iter fragmentation overhead ({FRAG_UPDATES}× stale records): {:.1}×",
...
fn main() {
    ...
    results.push(run_case("iter/fragmented", N as u64, || bench_iter(&frag_buf)));
    ...
    results.push(run_case("baseline/xxh3-long", N as u64, || {
        ...
        for i in 0..N as u64 {
            sum = sum.wrapping_add(xxh3::xxh3_64_with_seed(
                black_box(b"key_000042_some_longer_prefix___"), i));
        }
        black_box(sum);
        N as u64
    }));
