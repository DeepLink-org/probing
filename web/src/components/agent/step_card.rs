...
pub fn AgentPlaybookRunCard(
    ...
    rsx! {
        div {
            class: "rounded-lg border border-gray-200 bg-white border-l-4 border-l-blue-500 shadow-sm overflow-hidden",
            div { class: "px-3 py-2.5 bg-gradient-to-r from-blue-50/80 to-white border-b border-gray-100",
                div { class: "flex items-start gap-2",
                    Icon { icon: &icondata::AiRobotOutlined, class: "w-4 h-4 text-blue-600 mt-0.5 shrink-0" }
                    div { class: "min-w-0 flex-1",
                        div { class: "text-sm font-semibold text-gray-900", "{title}" }
                        div { class: "text-[10px] text-gray-500 font-mono mt-0.5",
                            "{category} · {playbook_id}"
                        }
                    }
                }
            }
            if !docs_preview.is_empty() {
                p { class: "px-3 py-2 text-xs text-gray-600 whitespace-pre-wrap leading-relaxed",
                    "{docs_preview}"
                }
            }
        }
...
pub fn AgentStepCard(step: AgentStepCardData) -> Element {
    ...
    rsx! {
        div {
            class: "rounded-lg border border-gray-200 bg-white border-l-4 {accent} shadow-sm overflow-hidden",
            button {
                ...
                div { class: "flex items-center gap-2 min-w-0",
                    Icon { icon, class: "w-4 h-4 text-gray-600 shrink-0" }
                    ...
                    div { class: "flex items-center gap-1 shrink-0",
                        span {
                            class: "inline-flex px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wide border {badge_cls}",
                            "{badge_label}"
                        }
                ...
                div { class: "px-3 py-2 space-y-2",
                    ...
                    if let Some(ref view) = navigate_view {
                        ...
                        if step.row_count.unwrap_or(0) > 0 {
                            ...
                            p { class: "text-xs text-amber-800 bg-amber-50 rounded-md px-2 py-1.5 border border-amber-100",
                                "{step.body_text}"
                            }
                        }
                    } else if !step.body_text.is_empty() {
                        if matches!(step.kind, AgentStepKind::Api) {
                            ...
                            p { class: "text-xs text-gray-700 whitespace-pre-wrap", "{step.body_text}" }
                        }
                    }
