...
def span(*args, **kwargs):
    ...
    if len(args) == 1 and isinstance(args[0], str):
        ...
        class SpanWrapper:
            def __init__(self, name: str, kind: Optional[str], code_path: Optional[str], attrs: dict):
                self.name = name
                self.kind = kind
            ...
            def __call__(self, func: Callable) -> Callable:
                ...
                def wrapper(*wargs, **wkwargs):
                    ...
                    if self.attrs:
                        ...
                        with span(self.name, kind=self.kind, code_path=self.code_path, **self.attrs) as s:
                            return func(*wargs, **wkwargs)
                    else:
                        with span_raw(self.name, kind=self.kind, code_path=self.code_path) as s:
                            return func(*wargs, **wkwargs)
                return wrapper
            ...
            def __enter__(self):
                ...
                if parent:
                    self._span = Span.new_child(parent, self.name, kind=self.kind, code_path=self.code_path)
                else:
                    self._span = Span(self.name, kind=self.kind, code_path=self.code_path)
