  ...
  %pytorch memory                          # Show CUDA memory info
    ...
    def _cmd_models(self) -> None:
        ...
        try:
            ...
            print(
                f"To profile a specific module, use: %pytorch profile mid=<module_id>"
            )
        except Exception as e:
    ...
    def install_profiler(module, steps: int = 1):
        ...
        class _Profiler:
            ...
            def __init__(self, steps: int) -> None:
                ...
                self._module = None
                self._status = False
                self._profiler_started = (
                    False  # Track if profiler was started by this instance
                )
            ...
            def pre_forward_hook(self, module, input):
                ...
                if not self._status and not self._profiler_started:
                    try:
                        print(
                            f"==== Start profiling module {module.__class__.__name__} ===="
                        )
                        ...
                        if "already enabled" in str(e).lower():
                            ...
                            print(
                                f"Warning: Another profiler is running, skipping profiler for {module.__class__.__name__}"
                            )
                            self._status = (
                                True  # Mark as done to prevent further attempts
                            )
                            return None
                        else:
                            raise
            ...
            def post_forward_hook(self, module, input, output):
                ...
                if self._count >= self._steps and self._profiler_started:
                    try:
                        print(
                            f"==== Stop profiling module {module.__class__.__name__} after {self._steps} steps ===="
                        )
                        self._profiler.stop()
                        self._status = False
                        self._profiler_started = False
                        ...
                        if (
                            ...
                            print(
                                f"Warning: Profiler for {module.__class__.__name__} was already stopped"
                            )
                        self._status = False
                        self._profiler_started = False
            ...
            def summary(self):
                ...
                try:
                    ...
                    if events:
                        ...
                        print(
                            "Profiler has no events. Make sure the model has been executed."
                        )
                except Exception as e:
    ...
    def _profile(steps: int = 1, mid: Optional[int] = None):
        ...
        if mid is not None:
            ...
            if not tms:
                raise ValueError(
                    f"Module with ID {mid} not found. Use '%pytorch models' to list available modules."
                )
        else:
            ...
            if not tms:
                raise ValueError(
                    "No top-level modules found. Make sure you have PyTorch modules in memory."
                )
        ...
        for m in tms:
            ...
            if module_id in __main__.__probing__.get(TorchMagic.PROFILER_KEY, {}):
                print(
                    f"Warning: Profiler already exists for module {module_id}. Reinstalling..."
                )
