...
def _wrap_task_execution_in_worker():
    ...
    try:
        ...
        def traced_execute(*args, **kwargs):
            ...
            try:
                ...
                if task:
                    if hasattr(task, "actor_id") and task.actor_id:
                        ...
                        func_name = (
                            getattr(task, "function_name", None) or
            ...
            with tracer.start_as_current_span(span_name, kind=trace.SpanKind.INTERNAL, attributes=attributes) as span:
                try:
...
def _extract_time_from_events(events):
    ...
    for event in events:
        ...
        event_type = _get_attr(event, "event_type") or _get_attr(event, "type")
        time_ms = _get_attr(event, "time_ms") or _get_attr(event, "timestamp_ms") or _get_attr(event, "timestamp")
        ...
        if not start_time_ms and ("START" in event_type_str or "RUNNING" in event_type_str or "SCHEDULED" in event_type_str):
            start_time_ms = time_ms
        
        # Find end event (TASK_FINISHED, FAILED, etc.)
        if "FINISH" in event_type_str or "FAIL" in event_type_str or "CANCEL" in event_type_str or "END" in event_type_str:
...
def _convert_task_to_timeline_entry(task, index=0, total=1, worker_to_pid=None):
    ...
    func_name = _get_attr(task, "func_or_class_name") or _get_attr(task, "function_name") or _get_attr(task, "name") or "unknown_task"
    ...
    if start_time_ms is None:
        start_time_ms = _get_attr(task, "start_time_ms") or _get_attr(task, "creation_time_ms")
    ...
    duration = (end_time_ns - start_time_ns) if (start_time_ns and end_time_ns) else None
    ...
    if parent_task_id and str(parent_task_id) != "ffffffffffffffffffffffffffffffffffffffff01000000":
    ...
    return {
        ...
        "parent_id": hash(parent_task_id) if parent_task_id and str(parent_task_id) != "ffffffffffffffffffffffffffffffffffffffff01000000" else None,
        "kind": entry_type,
        "thread_id": 0,
...
def _convert_actor_to_timeline_entry(actor, worker_to_pid=None):
    ...
    class_name = _get_attr(actor, "class_name") or _get_attr(actor, "name") or "unknown_actor"
    ...
    # Fallback to direct time fields
    if start_time_ms is None:
        start_time_ms = _get_attr(actor, "start_time_ms") or _get_attr(actor, "creation_time_ms")
    if end_time_ms is None:
...
def get_ray_timeline_chrome_format(
    ...
    try:
        ...
        earliest_time = min(entry["start_time"] for entry in timeline if entry["start_time"])
        ...
        for process_id, worker_id in pid_to_worker.items():
            ...
            process_name = " | ".join(process_name_parts) if process_name_parts else f"Worker {process_id}"
        ...
        return json.dumps({"traceEvents": trace_events, "displayTimeUnit": "ms"}, indent=2)
    except Exception:
        import json
        return json.dumps({"traceEvents": []})
