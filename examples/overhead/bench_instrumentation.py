...
representative settings. Run inside a probing-injected process::
    ...
    PROBING=1 python examples/overhead/bench_instrumentation.py

Quick smoke (fewer iterations)::

    PROBING=1 python examples/overhead/bench_instrumentation.py --quick
...
``examples/overhead/run_nccl_bench.sh``). CPU pprof (SIGPROF) is server-side;
...
def _print_report_footer() -> None:
    ...
    print(
        "  • NCCL profiler: make nccl-profiler-bench  or  examples/overhead/run_nccl_bench.sh"
    )
