#!/usr/bin/env bash
# torchrun crash demo — run from repo root: ./examples/crash/run_torchrun.sh [mode] [crash-rank]
...
exec torchrun --standalone --nnodes=1 --nproc_per_node="$NPROC" \
  ...
  examples/crash/crash_torchrun_demo.py --mode "$MODE" --crash-rank "$CRASH_RANK"
